...
public partial class NewSaleView : UserControl
{
    ...
    {
        if (DataContext is NewSaleViewModel vm)
        {
    ...
    private void OnCatalogRowPreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        if (sender is not DataGridRow row ||
            ...
            DataContext is not NewSaleViewModel vm)
        {
            return;
        }
    ...
    {
        if (DataContext is not NewSaleViewModel vm)
        {
            return;
        }
        ...
        {
            if (vm.IsNormalSaleMode && vm.CompleteSaleCommand.CanExecute(null))
            {
                vm.CompleteSaleCommand.Execute(null);
                e.Handled = true;
            }
            else if (vm.IsThakaMode && vm.IssueMaterialToThakaCommand.CanExecute(null))
            {
                vm.IssueMaterialToThakaCommand.Execute(null);
                e.Handled = true;
            }
        }
        else if (e.Key == Key.F4 && vm.IsNormalSaleMode)
        {
