...
$runRoot = Join-Path $env:TEMP ('EdgeRetailsPhase1Pg_' + [Guid]::NewGuid().ToString('N'))
$data = Join-Path $runRoot 'data'
$pwFile = Join-Path $runRoot 'admin.pw'
$log = Join-Path $runRoot 'postgres.log'
New-Item -ItemType Directory -Path $runRoot -Force | Out-Null

$adminUser = 'er_p1_admin'
$testDb = 'edge_retails_phase1_test'
...
$started = $false
try {
    Write-Output 'PHASE1_PG_INIT_START'
    ...
    Write-Output 'PHASE1_PG_CREATEDB'
    ...
    Write-Output 'PHASE1_PG_INSPECT_SCHEMA'
    $inspectSql = @"
SELECT table_schema, table_name FROM information_schema.tables WHERE table_schema IN ('catalog', 'inventory', 'parties', 'finance', 'warranty') ORDER BY table_schema, table_name;
"@
    & (Join-Path $PgBin 'psql.exe') -h 127.0.0.1 -p $Port -U $adminUser -d $testDb -c $inspectSql

    Write-Output 'PHASE1_DISPOSABLE_POSTGRES_REHEARSAL_PASS'
}
finally {
