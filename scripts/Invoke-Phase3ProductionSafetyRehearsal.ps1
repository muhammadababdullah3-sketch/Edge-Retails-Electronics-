...
$runRoot = Join-Path $env:TEMP ('EdgeRetailsPhase3Pg_' + [Guid]::NewGuid().ToString('N'))
...
"@
    ...
    Write-Output 'PHASE3_PG_GRANT_RUNTIME_SCHEMA'
    $grantSql = @"
...
"@
    ...
    Write-Output 'PHASE3_PG_VERIFY_SCHEMA_TABLES'
    $tableCountSql = @"
...
"@
    ...
    if ([int]$tableCount -lt 62) {
        throw "Expected at least 62 canonical tables, but found $tableCount."
    }

    Write-Output 'PHASE3_PG_VERIFY_OUTBOX_TABLE'
    $outboxCheckSql = @"
SELECT count(*) FROM information_schema.tables WHERE table_schema = 'system' AND table_name = 'outbox_messages';
"@
    $outboxCount = (& (Join-Path $PgBin 'psql.exe') -h 127.0.0.1 -p $Port -U $adminUser -d $testDb -t -A -c $outboxCheckSql).Trim()
    if ([int]$outboxCount -ne 1) {
        throw "Outbox table system.outbox_messages not found in PostgreSQL database."
    }
    ...
    Write-Output 'PHASE3_PRODUCTION_SAFETY_REHEARSAL_PASS'
}
finally {
